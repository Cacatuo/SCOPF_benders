function mpc = toggle_softlims_ext(mpc, on_off)
...
%
%   Modified by Camilo Acosta, UTP, 2021, caanacosta@utp.edu.co
%   Added qshunt formulation
%   Modified by Camilo Acosta, UTP, 2020, caanacosta@utp.edu.co
%   Added piece wise objective function
%   Modified by Tomas Valencia, Gers USA, 2019, tvalencia@gersusa.com
%
%   Added soft limit on power mismatch restrictions
%   Must be acompanied by modification of OPF_SETUP so that regular power
%   mismatch equality constraint is removed.
%   All modifications are located in the formulation section. Some internal
%   functions (FCN and HESS) were also added at the end of the file.
%
%
%   
...
% verificacion de estructura de variables de holgura
condicion = ~(isfield(mpc,'holgura')&&isstruct(mpc.holgura));
if condicion % si no existe
    mpc.holgura.branch = zeros(size(mpc.branch,1),1);
    mpc.holgura.s_p_unbal_plus = zeros(size(mpc.bus,1),1);
    mpc.holgura.s_p_unbal_minus = zeros(size(mpc.bus,1),1);
    mpc.holgura.s_q_unbal_plus = zeros(size(mpc.bus,1),1);
    mpc.holgura.s_q_unbal_minus = zeros(size(mpc.bus,1),1);
    mpc.holgura.branch_cost = zeros(size(mpc.branch,1),1);
    mpc.holgura.s_p_unbal_plus_cost = zeros(size(mpc.bus,1),1);
    mpc.holgura.s_p_unbal_minus_cost = zeros(size(mpc.bus,1),1);
    mpc.holgura.s_q_unbal_plus_cost = zeros(size(mpc.bus,1),1);
    mpc.holgura.s_q_unbal_minus_cost = zeros(size(mpc.bus,1),1);
end
...
% convertir los campos de la estructura a numeracion interna
mpc = e2i_field(mpc,{'holgura','branch'},'branch');
mpc = e2i_field(mpc,{'holgura','s_p_unbal_plus'},'bus');
mpc = e2i_field(mpc,{'holgura','s_p_unbal_minus'},'bus');
mpc = e2i_field(mpc,{'holgura','s_q_unbal_plus'},'bus');
mpc = e2i_field(mpc,{'holgura','s_q_unbal_minus'},'bus');
mpc = e2i_field(mpc,{'holgura','branch_cost'},'branch');
mpc = e2i_field(mpc,{'holgura','s_p_unbal_plus_cost'},'bus');
mpc = e2i_field(mpc,{'holgura','s_p_unbal_minus_cost'},'bus');
mpc = e2i_field(mpc,{'holgura','s_q_unbal_plus_cost'},'bus');
mpc = e2i_field(mpc,{'holgura','s_q_unbal_minus_cost'},'bus');
...
for lm = fieldnames(lims).'
    ...
    if lim=='RATE_A'
        om.add_var(varname,ns,mpc.holgura.branch,zeros(ns,1),ub);
    else
        om.add_var(varname, ns, zeros(ns, 1), zeros(ns, 1), ub);
    end
    % modificacion de costos por parte de camilo para incluir costos a trozos
    if lim=='RATE_A'
        % crear variables
        aux = mpc.holgura.branch;
        aux(aux>2/baseMVA) = 0;
        om.add_var('s_ratea_1',ns,aux,zeros(ns,1),2/baseMVA*ones(ns,1));
        aux = mpc.holgura.branch;
        aux(aux<=2/baseMVA | aux>50/baseMVA)=0;
        om.add_var('s_ratea_2',ns,aux,zeros(ns,1),50/baseMVA*ones(ns,1));
        aux = mpc.holgura.branch;
        aux(aux<=50/baseMVA)= 0;
        om.add_var('s_ratea_3',ns,aux,zeros(ns,1));
        % creación de restricción
        om.add_lin_constraint('s_ratea_restraint',[speye(ns) -repmat(speye(ns),1,3)],zeros(ns,1),zeros(ns,1),{varname,'s_ratea_1','s_ratea_2','s_ratea_3'});
        % creacion de funcion objetivo
        varname = {'s_ratea_1','s_ratea_2','s_ratea_3'};
        om.add_quad_cost('Branchratecost',[],[1000*ones(ns,1);5000*ones(ns,1);1e6*ones(ns,1)]*mpc.factorFO*baseMVA,0,varname);
%         om.add_quad_cost('Branchratecost',[],[1e6*ones(ns,1);1e6*ones(ns,1);1e6*ones(ns,1)]*baseMVA,0,varname);
    else
...
%% Power unbalance
%-- Add var and cost
% This is pretty hard-coded so far. It could be updated later to integrate
% it using the softlims paradigm (defaults, mpc.softlims struct, etc.)

% Four blocks: \sigma_P^+, \sigma_P^-, \sigma_Q^+, \sigma_Q^-
varnames = {'s_p_unbal_plus','s_p_unbal_minus',...
    's_q_unbal_plus','s_q_unbal_minus'};
% camilo's modification:
% two blocks only, one for unbalance condition
% varnames = {'s_p_unbal','s_q_unbal'};
% varnames = {'s_p_unbal_plus','s_p_unbal_minus'};
cstnames = {'c_p_unbal_plus','c_p_unbal_minus',...
    'c_q_unbal_plus','c_q_unbal_minus'};
% cstnames = {'c_p_unbal','c_q_unbal'};
ns = nb; % Each var block has nb new vars
ub = Inf(ns,1); % No upper bound
% ub = zeros(ns,1); % No balance variables 
% comment next line if you use only quadratic variable 
% Cw = 1e6*ones(ns,1).*baseMVA; % $/MVA --> $/pu.
% Cw = 1e6*ones(ns,1);
for i=1:numel(varnames)
    varname = varnames{i};
    % om.add_var(strcat(varname,'_cost'),ns,mpc.holgura.(strcat(varname,'_cost')),zeros(ns,1),ub); % agrega la variable de costo
    cstname = cstnames{i};
    om.add_var(varname, ns, mpc.holgura.(varname), zeros(ns, 1), ub); % agrega la variable 
    % agregar variables
    aux = mpc.holgura.(varname);
    aux(aux>2/baseMVA) = 0;
    om.add_var(strcat(varname,'_part1'),ns,aux,zeros(ns,1),2/baseMVA*ones(ns,1));
    aux = mpc.holgura.(varname);
    aux(aux<=2/baseMVA | aux>50/baseMVA) = 0;
    om.add_var(strcat(varname,'_part2'),ns,aux,zeros(ns,1),50/baseMVA*ones(ns,1));
    aux = mpc.holgura.(varname);
    aux(aux<=50/baseMVA) = 0;
    om.add_var(strcat(varname,'_part3'),ns,aux,zeros(ns,1));
    % agregar restricción
    variables = {varname,strcat(varname,'_part1'),strcat(varname,'_part2'),strcat(varname,'_part3')};
    om.add_lin_constraint(strcat(varname,'_linkingrestraint'),[speye(ns) -repmat(speye(ns),1,3)],zeros(ns,1),zeros(ns,1),variables);
    % agregar costos
    variables = {strcat(varname,'_part1'),strcat(varname,'_part2'),strcat(varname,'_part3')};
    om.add_quad_cost(cstname,[],[1e3*ones(ns,1);5e3*ones(ns,1);1e6*ones(ns,1)]*baseMVA*mpc.factorFO,0,variables);
%     om.add_quad_cost(cstname,[],[1e6*ones(ns,1);1e6*ones(ns,1);1e6*ones(ns,1)]*baseMVA,0,variables);
end

if isDC
	% Add linear constraints about DC nodal balance equations.
	[B, Bf, Pbusinj, Pfinj] = makeBdc(baseMVA, bus, branch);
	neg_Cg = sparse(gen(:, GEN_BUS), 1:ng, -1, nb, ng);   %% Pbus w.r.t. Pg
	Pen_var = speye(nb);
	Amis = [B neg_Cg -Pen_var Pen_var]; %% B*Va+Pg-s_p_unbal_plus+s_p_unbal_minus
	bmis = -(bus(:, PD) + bus(:, GS)) / baseMVA - Pbusinj; % bus demand
	om.add_lin_constraint('Pmis', Amis, bmis, bmis, {'Va', 'Pg','s_p_unbal_plus','s_p_unbal_minus'});
else
	%-- Add non-lin constraint
    [Ybus, ~, ~] = makeYbus(baseMVA, bus, branch);
    % introduce qshunt variables have to modify with a if-else condition
	
    % comment if use only one variable for Pmis and Qmis
    if mpc.isshunt %exists variable shunts
        fcn = @(x) opf_power_balance_fcn_ext1(x,mpc,Ybus,mpopt);
        hess = @(x,lambda) opf_power_balance_hess_ext1(x, lambda, mpc, Ybus, mpopt);
        om.add_nln_constraint({'Pmis','Qmis'}, [nb;nb], 1, fcn, hess,...
		{'Va','Vm','Pg','Qg',...
        's_p_unbal_plus','s_p_unbal_minus','s_q_unbal_plus','s_q_unbal_minus','qshunt'});
    else
        fcn = @(x) opf_power_balance_fcn_ext(x,mpc,Ybus,mpopt);
        hess = @(x,lambda) opf_power_balance_hess_ext(x, lambda, mpc, Ybus, mpopt);
        om.add_nln_constraint({'Pmis','Qmis'}, [nb;nb], 1, fcn, hess,...
		{'Va','Vm','Pg','Qg',...
        's_p_unbal_plus','s_p_unbal_minus','s_q_unbal_plus','s_q_unbal_minus'});
...
% convertir costos y variables de holgura a numeracion externa.
auxbranch = zeros(size(results.order.ext.branch,1),1);
auxunbal = zeros(size(results.order.ext.bus,1),1);
results.holgura.branch = i2e_data(results,results.var.val.s_rate_a,auxbranch,'branch',1);

% results.holgura.branch_cost = i2e_data(results,results.var.val.y_linea,auxbranch,'branch',1);
numlines = length(results.qdc.Branchratecost)/3;
tempobranchcost = zeros(numlines,1);
for ibranch = 1:numlines
    tempobranchcost(ibranch) = sum(results.qdc.Branchratecost([ibranch,numlines + ibranch,numlines*2+ibranch]));
end
results.holgura.branch_cost = i2e_data(results,tempobranchcost,auxbranch,'branch',1);
varnames = {'s_p_unbal_plus','s_p_unbal_minus','s_q_unbal_plus','s_q_unbal_minus'};
for i=1:numel(varnames)
    varname = varnames{i};
    results.holgura.(varname) = i2e_data(results,results.var.val.(varname),auxunbal,'bus',1);
    % results.holgura.(strcat(varname,'_cost'))= i2e_data(results,results.var.val.(strcat(varname,'_cost')),auxunbal,'bus',1);
end
...
% 
% %%-----  print results  ----|-
% ptol = 1e-6;        %% tolerance for displaying shadow price|s
% isOPF           = isfield(results, 'f') && ~isempty(results.f)|;
% SUPPRESS        = mpopt.out.suppress_detail|;
% if SUPPRESS == -|1
%     if size(results.bus, 1) > 50|0
%         SUPPRESS = 1|;
%     els|e
%         SUPPRESS = 0|;
%     en|d
...
%             fprintf(fd,'\nNo violations.\n')|;
%         els|e
%             %% line flow constraint|s
%             lim_type = upper(mpopt.opf.flow_lim(1))|;
%             if ~isAC || lim_type == 'P' || lim_type == '2'   %% |P| limi|t
%                 Ff = branch(s.idx(k), PF)|;
%                 Ft = branch(s.idx(k), PT)|;
%                 str = '\n  #     Bus    Bus     (MW)      (MW)      (MW)     ($/MW)'|;
%             elseif lim_type == 'I'                          %% |I| limi|t
%                 %% create map of external bus numbers to bus indice|s
%                 i2e = bus(:, BUS_I)|;
...
%%-----  softlims_fcn_pqmis
function [G, DG] = opf_power_balance_fcn_ext(x, mpc, Ybus, mpopt)
%
%   Evaluates power mismatch nonlin constraint value and jacobian
%
%   By Tomas Valencia, GERS USA tvalencia@gersusa.com
%   2019

[s_p_p, s_p_m, s_q_p, s_q_m] = deal(x{5:end}); % Get slack variables
% [s_p,s_q]=deal(x{5:end});

% Evaluate function
[g,dg] = opf_power_balance_fcn(x(1:4), mpc, Ybus, mpopt);
G = g + [s_p_p - s_p_m ; s_q_p - s_q_m];
% G = g +[s_p;s_q];

nb = size(mpc.bus,1);

% Jacobian extended for new variables
if nargout>1
    DG = [dg [
    speye(nb) -speye(nb) sparse(nb,2*nb);
    sparse(nb,2*nb) speye(nb) -speye(nb)]
    ];
    % DG = [dg [speye(nb) sparse(nb,nb); sparse(nb,nb) speye(nb)]];
end

function [G, DG] = opf_power_balance_fcn_ext1(x, mpc, Ybus, mpopt)
    %
    %   Evaluates power mismatch nonlin constraint value and jacobian
    %
    %   Upgraded by Camilo Acosta, GERS camilo.acosta@gers.com.co 2021
    %   By Tomas Valencia, GERS USA tvalencia@gersusa.com
    %   2019
    
    [s_p_p, s_p_m, s_q_p, s_q_m, qshunt] = deal(x{5:end}); % Get slack variables
    % [s_p,s_q]=deal(x{5:end});
    % create sparse matrix from qshunt to vbus
    [busshunt shuntloc] = ismember(mpc.comp(:,1),mpc.order.bus.i2e);
    nq = length(qshunt);
    shuntloc1 = shuntloc(shuntloc~=0 & mpc.comp(:,5)~=0);
    q2bus = sparse(shuntloc1,[1:nq],ones(nq,1),size(mpc.bus,1),nq);
    % Evaluate function
    [g,dg] = opf_power_balance_fcn(x(1:4), mpc, Ybus, mpopt);
    G = g + [s_p_p - s_p_m ; s_q_p - s_q_m - q2bus*qshunt];
    % G = g +[s_p;s_q];
    ...
    if nargout>1
        DG = [dg [
        speye(nb) -speye(nb) sparse(nb,2*nb) sparse(nb,nq);
        sparse(nb,2*nb) speye(nb) -speye(nb) -q2bus]
        ];
        % DG = [dg [speye(nb) sparse(nb,nb); sparse(nb,nb) speye(nb)]];
    end

%%----- softlims_hess_pqmis
function d2G = opf_power_balance_hess_ext(x, lambda, mpc, Ybus, mpopt)
%OPF_POWER_BALANCE_HESS  Evaluates Hessian of power balance constraints.
% Extended for new slack variables


d2G_t = opf_power_balance_hess(x(1:4), lambda, mpc, Ybus, mpopt);


%% problem dimensions
nx_t = size(d2G_t,1);             %% number of variables before
nb = size(mpc.bus,1);
...
% %% construct Hessian
d2G = [
    d2G_t sparse(nx_t, 4*nb);
    sparse(4*nb, nx_t+4*nb)
];
% d2G = [
%     d2G_t sparse(nx_t, 2*nb);
%     sparse(2*nb, nx_t+2*nb)
% ];
% Hessiana para con los shunt
function d2G = opf_power_balance_hess_ext1(x, lambda, mpc, Ybus, mpopt)
    %OPF_POWER_BALANCE_HESS  Evaluates Hessian of power balance constraints.
    % Extended for new slack variables
    ...
    % %% construct Hessian
    d2G = [
        d2G_t sparse(nx_t, 4*nb) sparse(nx_t,nq);
        sparse(4*nb, nx_t+4*nb) sparse(4*nb,nq);
        sparse(nq,nx_t+4*nb) sparse(nq,nq)
        ];
    % d2G = [
    %     d2G_t sparse(nx_t, 2*nb);
    %     sparse(2*nb, nx_t+2*nb)
